...
\end{equation}
% \begin{equation}
% \tilde{g}:=g-\left(g(V,V)\right)^{-1}\alpha\otimes\alpha\,,
% \end{equation}
% Wir können $T_p(E/G)=T_p E/\mathfrak{g}$ mit $\langle V(p)
% \rangle^\perp$
% identifizieren indem wir den Vektorraum-Isomorphismus
% \begin{equation}
% \begin{split}
% \Psi:T_{\pi(p)}(E/G)&\to\langle V(p)
% \rangle^\perp\\
% [X]&\mapsto X-\frac{g(X,V)V}{g(V,V)}
% \end{split}
% \end{equation}
% betrachten. Fassen wir also als Biliearform auf $T_pM$ auf. 
% Es ist zu zeigen, dass es sich dabei auch um eine Metrik handelt.
% Sei deshalb $X\in T_pM$ sodass für alle $Y\in T_pM$
% \begin{equation}
% \tilde{g}(X,Y)=0\,.
% \end{equation} 
% Sei $\tilde{Y}\in T_pE$, dann existieren $Y_1\in T_pM $,
% $Y_2=\alpha V $ und damit gilt
% \begin{equation}
% g(X,\tilde{Y})=g(X,Y_1)+g(X,Y_2)=\tilde{g}(X,Y_1)=0\,.
% \end{equation}
% Da $g$ nach voraussetzung nicht ausgeartet ist gilt dies auch für $\tilde{g}$.
...
% 5D Vakuum=> EH wirkung  L=sqrt(g) R unbekannt was \tensor{T}{_m_n} sein soll
\item die Metrik ist Extremum des EH-Funktionals
% ist equivalent zu vorherigem\ldots
...
der Ladung $Q$ eines Teilchens identifizieren.
 \section{Geodäten}
 \begin{bemerkung}[Umparametrisierung der Geodätengleichung]
...
%  Geodäten bewegen. 
%  Zunächst gilt weiterhin, dass diese proportional zur Bogenlänge parametrisiert
%  sind, d.h.
